    ...
    <aside className="sidebar hidden md:flex flex-col w-60 h-full">
      ...
      <div className="px-5 py-6 border-b border-white/[0.06]">
        ...
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-apple-sm bg-[var(--accent)] flex items-center justify-center flex-shrink-0">
        ...
        {NAV_ITEMS.filter((item) => !item.adminOnly || doctor?.role === "ADMIN").map(({ href, label, num }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={clsx(
                "sidebar-nav-item flex items-center gap-3 px-3 py-2.5 rounded-apple-md transition-all duration-100",
                active
                  ? "sidebar-nav-active"
                  : "text-[#666] hover:text-white hover:bg-white/5"
              )}
            >
              <span className="font-mono text-[10px] font-medium w-5 flex-shrink-0 opacity-50">{num}</span>
      ...
      <div className="px-3 pb-4 space-y-3 border-t border-white/[0.06] pt-4">
        <ThemeToggle />
        {doctor && (
          <div className="flex items-center gap-2.5 px-3 py-2 rounded-apple-md bg-white/[0.04] border border-white/[0.06]">
