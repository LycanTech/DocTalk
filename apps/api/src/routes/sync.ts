  ...
  try {
    ...
    for (const p of patients) {
      ...
      await prisma.patient.upsert({
        where: { id },
        update: { ...data as Record<string, unknown>, updatedAt: new Date() },
        create: { id, ...data as Record<string, unknown> },
      });
      results.patients++;
    }
    ...
    for (const r of records) {
      ...
      await prisma.medicalRecord.upsert({
        where: { id },
        update: { ...data as Record<string, unknown>, doctorId: req.doctor!.sub, updatedAt: new Date() },
        create: { id, ...data as Record<string, unknown>, doctorId: req.doctor!.sub },
      });
      results.records++;
    }
